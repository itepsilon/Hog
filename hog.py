...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    """bacon points we can get from opponent_score"""
    points = max(opponent_score // 10, opponent_score % 10) + 1

    """Taking advantage on free bacon if score >= 90"""
    if 100 - score <= points:
        return 0
    ...
    if score >= opponent_score:
        ...
        """Preventing Swine Swap effect"""
        if (score + 1) == opponent_score * 2:
            return 0
        if (score + points) == opponent_score * 2:
        	return 4

        """Double Preventing Swine Swap effect"""
        if score + 1 == 2 * (opponent_score + 1):
        	return 0

        if score + points == 2 *(opponent_score + 1):
        	return 4

        """Slow down the opponent"""
        if (score + opponent_score + points) % 7 == 0:
        	return 0
        
        if (score + opponent_score) % 7 == 0:
            """We are rolling 4 sided dice"""
            if (score+points)!=2*opponent_score and (score+points)!=2*(opponent_score+1) and points > 6:
            	return 0
            return 4
        else:
            """We are rolling 6 sided dice"""

            return swap_strategy(score, opponent_score)
    else:
        """We are losing the game"""

        """Taking advantage on Swine Swap"""
        ...
        if points >= 6 and opponent_score + score + points % 7 == 0:
        	return 0
