...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    while (score<goal) and (opponent_score <goal):
        ...
        if num_rolls==0 :
            score=score+take_turn(0,opponent_score,six_sided)
         
        else:
       
            dice=select_dice(score,opponent_score)
            score=score+take_turn(num_rolls,opponent_score,six_sided)
        ...
        if score<100:
            ...
            if num_rolls==0:
                opponent_score=opponent_score+take_turn(0,score,six_sided)
    
            else:
                dice=select_dice(score,opponent_score)
                opponent_score=opponent_score+take_turn(num_rolls,score,six_sided)
            if (score==2*opponent_score) or (opponent_score==2*score):
                score,opponent_score=opponent_score,score 
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    maxscore=1
    while a<=10:
        b=make_averaged(roll_dice,10000)(a,dice)
        print(a,'dice scores',b,'on average')
        if b>=maxscore:
            maxscore=b
            best_dice=a
        a=a+1
...
def run_experiments():
    ...
    if True: # Change to False to test bacon_strategy
...
def swap_strategy(score, opponent_score):
    ...
    b=opponent_score//10
    c=1+max(a,b)
    if (c+score)*2==opponent_score or c>=BACON_MARGIN and opponent_score*2!=c+score and opponent_score*2+1!=c+score:
        return 0
    else:
        return BASELINE_NUM_ROLLS
...
def final_strategy(score, opponent_score):
    ...
    if score <20, roll 5 dices and don't use this strategy
    Roll 10 dices if 2*(score+1)==opponent_score 
    Roll 0 if score+1=2*opponent_score 
    Roll 0 if score+2=2*opponent_score and opponent_score!=10 
    Roll 0 if dice=four_sided and roll 0 gives points>8 and roll 0 does not give points c
        such that c+score=2*opponent_score or c+score=2(opponent_score+1)
    Roll 3 if score>=1.5*opponent_score
    Roll 5 dices, if score<opponent_score and dice=six_sided
    Roll 4 dices, if 1.5*opponent_score>score>opponent_score and dice=six_side
    """
    ...
    #if score>2*opponent_score and c+score!=2(opponent_score) and c+score!=2(opponent_score+1):
     #  return 0


    
    if (c+score+opponent_score)%7==0 and c+score!=2(opponent_score) and c+score!=2(opponent_score+1):
        return 0
    else:
        return 5
